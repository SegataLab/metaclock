...
def refs_collector(alns_folder):
	...
	"""
	It takes a folder which contains individual alignments and writes all reference
...
def sub_que_matcher(sseq, qseq, s_idx, q_idx):
	...
	for i in range(len(sseq)):	
		# Here if, elif, else statement didn`t work properly????		
		if (sseq[i] != '-') and (qseq != '-'):
			s_idx_g =  s_idx + i - s_gap
			q_idx_g = q_idx + i - q_gap
			s_pos_lst.append(s_idx_g)
			q_pos_lst.append(q_idx_g)
		if (sseq[i] == '-') and (qseq[i] != '-'):
			s_gap += 1
		if (sseq[i] != '-') and (qseq[i] == '-'):
			q_gap += 1
		if (sseq[i] == '-') and (qseq[i] == '-'):
...
def homo_sites_finder(sstart, send, sseq, qstart, qend, qseq):
	...
	if ((send - sstart) > 0) & ((qend - qstart) > 0):
		...
		hit_alpha = sub_que_matcher(sseq, qseq, s_idx, q_idx)

		s_poses.extend(hit_alpha[0])
		q_poses.extend(hit_alpha[1])
...
def cols_sampled(all_alns, RefSeqs_SubWise_partition, nproc, coreness):
	ref_sub_groups = [] # The list which holds unique labels. Thus, each sub seq and que seq is paired. 
	all_cols_for_merging = [] # It hold all columns sampled with certain coreness
	all_cols = []
	...
	multi_sampling = nproc_sampler(nproc, ref_sub_groups) # It runs analysis in parallel
	multi_sampling_merge = defaultdict(list) # It merges a list of dictionaries into one dictionary
	
	for to_merge in multi_sampling:
		for single_work in to_merge:
			multi_sampling_merge[single_work].extend(to_merge[single_work])

	for i in multi_sampling_merge: # i=genome@position; e.g. GCA_002834235.fna@306351
		x = de_duplicator(multi_sampling_merge[i])
		if len(x)/len(aln_paths) >= coreness:
			all_cols_for_merging.append(x)

	return all_cols_for_merging
...
class output(object):
	...
	def output_merged(self, alnIO):
		...
		SeqIO.write(aln_builder(alnIO, consensus_col_lst), 'columned_merged_aln.fna', 'fasta')
	
	def output_individual(self, alnIO, aln_path):

		for i in range(len(aln_paths)):
			lst = [] # list of columns at one position  	
			for j in self.cols_list:
				if len(aln_paths) == len(j):
					lst.append(list(j[i]))
			
			final = aln_builder(alnIO, lst)
...
if __name__== '__main__':
	...
	Aln_IO = AlignIO.read('{}/{}'.format(pars['alns_folder'], refs_name_headers[0]), 'fasta')
	
	Cols_sampled = cols_sampled(aln_paths, RefSeqs_SubWise, pars['nproc'], pars['coreness'])

	opt = output(Cols_sampled)
	opt.output_merged(Aln_IO)
	opt.output_individual(Aln_IO, aln_paths)

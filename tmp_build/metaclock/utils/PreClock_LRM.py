...
if __name__ == '__main__':
	...
	tre = Tree(open(pars['raxml_bi_tree']).read().rstrip(), format = 2)
	mp = {i.strip().split('\t')[0]: i.strip().split('\t')[1] for i in open(pars['mp_file']).readlines()}
	tips_lst = [i.strip().split('\t')[0] for i in open(sys.argv[2]).readlines()]

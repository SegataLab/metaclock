...
def sampler_cds(args):
    ...
    sys.stdout.write('{} Sampling homologous sites between {} and {}....\n'.format(time_now(), seq_pair_1, seq_pair_2))
    sites_containner_sub = []
    col_sampled = ColSampler(seq_pair, matrice, gff_folder)
    no_col_sampled = str(len(col_sampled))
    sys.stdout.write('{} We found {} homologous sites between {} and {}\n'.format(time_now(), no_col_sampled, seq_pair_1, seq_pair_2))
    sys.stdout.write('{} Start locating site coordinates on the genome for {} and {}....\n'.format(time_now(), seq_pair_1, seq_pair_2))
...
def nproc_sampler(processor, ref_sub_lst, matrix, aln_dict, gff_folder = None):
    ...
    sys.stdout.write("{} The whole data has been partitioned and processed using {} processors.\n".format(time_now(), processor))
...
class column_toolkit(object):
    ...
    def merge_col_voting(self, majority_threshold = 0.51, number_refs = 2):
        ...
        sys.stdout.write('{} {} sites are shared by {} genomes.\n'.format(time_now(), homo_sites, number_refs))
        sys.stdout.write('{} Consensus site was decided by voting threshold {}\n'.format(time_now(), majority_threshold))
        return res
...
def main():
    ...
    sys.stdout.write('{} Check input alignments: {}....\n'.format(time_now(), alns))
    ...
    sys.stdout.write('{} Combining multiple alignments starts....\n'.format(time_now()))
    ...
    sys.stdout.write('{} Start building database for blastn....\n'.format(time_now()))
    db_file = build_db_file('contigs', refs_sequences, inter, 'samples') # Here argument 'samples' is just a place holder.
    sys.stdout.write('{} Database building is completed and database files prefix is: {}....\n'.format(time_now(), db_file))

    sys.stdout.write('{} Start searching for homologous regions among reference genomes....\n'.format(time_now()))
    blastn_threads = blastn_nproc(pars['nproc'])
    raw_blastn_opt = blast_genomes(inter, db_file, refs_sequences, blastn_threads, True)
    sys.stdout.write('{} Search is completed and the raw results are in:{}\n'.format(time_now(), raw_blastn_opt))

    sys.stdout.write('{} Start cleaning raw results from homologs searching....\n'.format(time_now()))
    cleaned_blastn = QC_on_blastn(raw_blastn_opt, pars['length'], pars['identity'], inter)
    sys.stdout.write('{} Cleaning is completed and the results are in {}....\n'.format(time_now(), cleaned_blastn))

    sys.stdout.write('{} Start partitioning data for speeding up computtaion using multiple processors....\n'.format(time_now()))

    partioned_matrice = {pair: partition_genomes(cleaned_blastn)[pair] for pair in ref_sub_groups}


    if pars['CDS_region']:
        sys.stdout.write("{} Combining multiple aligbments using only protein coding sites....\n".format(time_now()))
        sys.stdout.write('{} This process would take a bit long....\n'.format(time_now()))
        ...
        sys.stdout.write("{} Combining multiple aligbments using all sites....\n".format(time_now()))
        sys.stdout.write("{} This process would take a bit long....\n".format(time_now()))
        all_sites_nproc = nproc_sampler(pars['nproc'], ref_sub_groups, partioned_matrice, aln_dict)
    
    sys.stdout.write('{} Start cleaning up homologous sites found....\n'.format(time_now()))
    sites_containner = creat_uniqe_columns(all_sites_nproc)
    sys.stdout.write('{} Sites cleaning is completed!\n'.format(time_now()))

    sys.stdout.write('{} Start building consensus sites\n'.format(time_now()))
    col_tool_obj = column_toolkit(sites_containner)
    sys.stdout.write('{} Start building consensus alignment....\n'.format(time_now()))
    cols_merged = col_tool_obj.merge_col_voting(pars['voting_threshold'], pars['homo_site_in_refs'])
    sys.stdout.write('{} Consensus alignment reconstruction is completed!\n'.format(time_now()))
    ...
    sys.stdout.write('{} Please check your consensus alignment in: {}\n'.format(time_now(), opt_file))
